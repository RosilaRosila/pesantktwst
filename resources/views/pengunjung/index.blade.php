            ...
            <div class="content-header">
                ...
                <div class="content-header">
                    <div class="container-fluid">
                        <h4 class="m-0 text-dark">Kelola Data Pengunjung</h4>
                        <br>
                        <br>
                        ...
                        <section class="content">
                            <div class="card">
                                <div class="card-header">
                                    ...
                                    <form method="GET" action="{{ route('datapengunjung.index') }}" class="form-inline mb-3">
                                ...
                                <div class="card-body table-responsive">
                                    <table class="table table-hover table-bordered text-sm">
                                        ...
                                        <tbody>
                                            @if($datapengunjung->count() > 0)
                                            @foreach($datapengunjung as $dp)
                                            <tr>
                                                <th style="text-align: center;">
                                                    @if($datapengunjung instanceof \Illuminate\Pagination\LengthAwarePaginator)
                                                    {{ $loop->iteration + ($datapengunjung->currentPage() - 1) * $datapengunjung->perPage() }}
                                                    @else
                                                    {{ $loop->iteration }}
                                                    @endif
                                                </th>
                                                ...
                                                <td style="text-align: center;">
                                                    <a href="{{ route('datapengunjung.destroy', $dp->id) }}" class="btn btn-danger btn-sm" style="font-size:11px">Delete</a>
                                                </td>
                                            </tr>
                                            @endforeach
                                            @else
                                    ...
                                    <div>
                                        <div class="float-left" style="font-size: 12px; font-weight:bold">
                                            @if ($pagination)
                                            Showing
                                            {{$datapengunjung->firstItem() }}
                                            to
                                            {{ $datapengunjung->lastItem() }}
                                            of
                                            {{ $datapengunjung->total() }}
                                            entries
                                        </div>
                                        <div class="float-right">
                                            {!! $datapengunjung->appends(['perPage' => request('perPage'), 'search' => request('search')])->render() !!}
                                        </div>
                                        @else
                                        Showing all {{ $datapengunjung->count() }} entries
                                        @endif
                                    </div>
                                </div>
                            </div>
