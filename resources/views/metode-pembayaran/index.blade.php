            ...
            <div class="content-header">
                ...
                <div class="content-header">
                    <div class="container-fluid">
                        <h4 class="m-0 text-dark">Kelola Data Metode Pembayaran</h4>
                        <br>
                        <br>
                        <div class="text-end mb-5">
                            @can('metodepembayaran-create')
                            <a class="btn btn-primary" href="{{ route('metodepembayaran.create') }}">
                                <i class=" nav-icon fas fa-plus-circle"></i>&nbsp Tambah Metode Pembayaran
                            </a>
                            @endcan
                        </div>
                        ...
                        <section class="content">
                            <div class="card">
                                <div class="card-header">
                                    ...
                                    <form method="GET" action="{{ route('metodepembayaran.index') }}" class="form-inline mb-3">
                                ...
                                <div class="card-body table-responsive">
                                    <table class="table table-hover table-bordered text-sm">
                                        ...
                                        <tbody>
                                            @if($datametode->count() > 0)
                                            @foreach($datametode as $dm)
                                            <tr>
                                                <th style="text-align: center;">
                                                    @if($datametode instanceof \Illuminate\Pagination\LengthAwarePaginator)
                                                    {{ $loop->iteration + ($datametode->currentPage() - 1) * $datametode->perPage() }}
                                                    @else
                                                    {{ $loop->iteration }}
                                                    @endif
                                                </th>
                                                <td style="font-size: 12px;">{{ $dm->name }}</td>
                                                <td style="text-align: center;">
                                                    @can('metodepembayaran-edit')
                                                    <a href="{{ route('metodepembayaran.edit', $dm->id) }}" class=" btn btn-primary btn-sm" style="color: white; font-size:11px">Edit</a>
                                                    @endcan

                                                    @can('metodepembayaran-delete')
                                                    <a href="{{ route('metodepembayaran.destroy', $dm->id) }}" class="btn btn-danger btn-sm" style="font-size:11px">Delete</a>
                                                    @endcan
                                                </td>
                                            </tr>
                                            @endforeach
                                    ...
                                    <div>
                                        ...
                                        <div class="float-right">
                                            {!!$datametode->appends(['perPage' => request('perPage'), 'search' => request('search')])->render() !!}
                                        </div>
                                        @else
                                        Showing all {{ $datametode->count() }} entries
                                        @endif
                                    </div>
                                </div>
                            </div>
