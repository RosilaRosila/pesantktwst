            ...
            <div class="content-header">
                ...
                <div class="content-header">
                    <div class="container-fluid">
                        <h4 class="m-0 text-dark">Kelola Data Fasilitas</h4>
                        <br>
                        <br>
                        <div class="text-end mb-5">
                            @can('fasilitas-create')
                            <a class="btn btn-primary" href="{{ route('fasilitas.create') }}">
                                <i class=" nav-icon fas fa-plus-circle"></i>&nbsp Tambah Data Fasilitas
                            </a>
                            @endcan
                        </div>
                        ...
                        <section class="content">
                            <div class="card">
                                <div class="card-header">
                                    ...
                                    <form method="GET" action="{{ route('fasilitas.index') }}" class="form-inline mb-3">
                                ...
                                <div class="card-body table-responsive">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <th style="text-align: center;">
                                                    @if($datafasilitas instanceof \Illuminate\Pagination\LengthAwarePaginator)
                                                    {{ $loop->iteration + ($datafasilitas->currentPage() - 1) * $datafasilitas->perPage() }}
                                                    @else
                                                    {{ $loop->iteration }}
                                                    @endif
                                                </th>
                                                <td style="text-align: center;">
                                                    <img src="{{ asset($fs->imgicon) }}" alt="" height="25px">
                                                </td>
                                                <td style="font-size: 12px;">{{ $fs->namaicon }}</td>
                                                <td style="text-align: center;">
                                                    @can('fasilitas-edit')
                                                    <a href="{{ route('fasilitas.edit', $fs->id) }}" class=" btn btn-primary btn-sm" style="color: white; font-size:11px">Edit</a>
                                                    @endcan

                                                    @can('fasilitas-delete')
                                                    <a href="{{ route('fasilitas.destroy', $fs->id) }}" class="btn btn-danger btn-sm" style="font-size:11px">Delete</a>
                                                    @endcan
                                                </td>
                                            </tr>
                                            @endforeach
                                    ...
                                    <div>
                                        ...
                                        <div class="float-right">
                                            {!! $datafasilitas->appends(['perPage' => request('perPage'), 'search' => request('search')])->render() !!}
                                        </div>
                                        @else
                                        Showing all {{ $datafasilitas->count() }} entries
                                        @endif
                                    </div>
                                </div>
                            </div>
