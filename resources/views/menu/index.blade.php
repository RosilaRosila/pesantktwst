            ...
            <div class="content-header">
                ...
                <div class="content-header">
                    <div class="container-fluid">
                        <h4 class="m-0 text-dark">Kelola Data Permissions</h4>
                        <br>
                        <br>
                        <div class="text-end mb-5">
                            @can('menu-create')
                            <a class="btn btn-primary" href="{{ route('menu.create') }}">
                                <i class=" nav-icon fas fa-plus-circle"></i>&nbsp Tambah Data Permissions
                            </a>
                            @endcan
                        </div>
                        ...
                        <section class="content">
                            <div class="card">
                                <div class="card-header">
                                    ...
                                    <form method="GET" action="{{ route('menu.index') }}" class="form-inline mb-3">
                                ...
                                <div class="card-body table-responsive">
                                    <table class="table table-hover table-bordered text-sm" style="font-size: 8px;">
                                        ...
                                        <tbody>
                                            ...
                                            <tr style="font-size: 12px;">
                                                <th style="text-align: center;">
                                                    @if($getMenu instanceof \Illuminate\Pagination\LengthAwarePaginator)
                                                    {{ $loop->iteration + ($getMenu->currentPage() - 1) * $getMenu->perPage() }}
                                                    @else
                                                    {{ $loop->iteration }}
                                                    @endif
                                                </th>
                                                <td>{{ $product->name }}</td>
                                                <td style="text-align:center">{{ $product->guard_name }}</td>
                                                <td style="text-align: center;">
                                                    <form action="" method="POST">
                                                        <a class="btn btn-info btn-sm" href="{{ route('menu.show', $product->id) }}">Show</a>
                                                        @can('menu-edit')
                                                        <a class="btn btn-primary btn-sm" href="{{ route('menu.edit', $product->id) }}">Edit</a>
                                                        @endcan

                                                        @can('menu-delete')
                                                        <a class="btn btn-danger btn-sm" href="{{ route('menu.delete', $product->id) }}">Delete</a>
                                                        @endcan
                                                    </form>
                                                </td>
                                            </tr>
                                    ...
                                    <div>
                                        ...
                                        <div class="float-right">
                                            {!! $getMenu->appends(['perPage' => request('perPage'), 'search' => request('search')])->render() !!}
                                        </div>
                                        @else
                                        Showing all {{ $getMenu->count() }} entries
                                        @endif
                                    </div>
                                </div>
                            </div>
